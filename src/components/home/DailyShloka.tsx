...
export const DailyShloka: React.FC = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-stone-900 via-stone-900/90 to-stone-950 border border-amber-800/30 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-amber-900/15 pb-6">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/15 text-amber-200 text-xs font-serif font-semibold border border-amber-800/20">
            <Diya size="sm" />
            <span>दैनिकम् श्लोकम् मन्त्रकोषश्च • Daily Shloka & Mantra Treasury</span>
          </div>
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-amber-100 sanskrit-font">
            Divine Shlokas & Gods-Goddesses Mantra Explorer
          </h2>
          <p className="text-xs sm:text-sm text-stone-300 font-serif max-w-2xl">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 bg-stone-800/80 p-1.5 rounded-2xl border border-amber-800/20 text-xs font-serif font-medium self-start md:self-auto">
          <button
            onClick={() => setActiveTab('daily')}
            className={`px-3.5 py-2 rounded-xl transition-all flex items-center gap-1.5 ${
              activeTab === 'daily'
                ? 'bg-amber-700 text-white font-bold shadow-md'
                : 'text-stone-300 hover:bg-amber-500/20'
            }`}
          >
      ...
      {activeTab === 'daily' && (
        <div className="space-y-6">
          <div className="bg-stone-900/90 border border-amber-800/20 rounded-2xl p-6 sm:p-8 shadow-inner space-y-6 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-amber-900/10 pb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2.5 rounded-xl bg-amber-500/20 text-amber-200 hover:bg-amber-500/30 font-serif text-xs font-semibold flex items-center gap-1.5 transition-all min-h-[44px]"
                >
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl bg-amber-500/20 text-amber-200 hover:bg-amber-500/30 transition-all min-w-[44px] min-h-[44px] flex items-center justify-center"
                >
                ...
                >
                  <Bookmark
                    className={`w-4 h-4 ${
                      isBookmarked(`daily-shloka-${currentShloka.title}`) ? 'fill-current text-amber-400' : ''
                    }`}
                  />
                </button>
              </div>
            ...
            <div className="p-6 bg-gradient-to-br from-stone-800/90 to-stone-900/90 rounded-2xl border border-amber-800/25 text-center space-y-3 shadow-md">
              <p className="font-serif text-2xl sm:text-3xl font-bold text-amber-100 sanskrit-font whitespace-pre-line leading-relaxed">
                {currentShloka.sanskrit}
              </p>

              {currentShloka.transliteration && (
                <p className="text-xs sm:text-sm text-amber-300/90 italic font-serif">
                  {currentShloka.transliteration}
                </p>
              )}
            </div>
            ...
            {currentShloka.sourceScripture && (
              <div className="text-right text-[11px] text-amber-400 font-serif font-medium pt-2 border-t border-amber-900/10">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between bg-stone-900/80 p-4 rounded-2xl border border-amber-800/20">
            ...
            <div className="relative w-full sm:w-auto min-w-0 sm:min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 bg-stone-800 border border-amber-800/20 rounded-xl text-xs text-stone-200 focus:outline-none focus:ring-1 focus:ring-amber-600 min-h-[38px] placeholder:text-stone-400"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredMantras.map(m => {
              ...
              return (
                <div
                  key={m.id}
                  className="bg-stone-900/90 border border-amber-800/20 rounded-2xl p-5 shadow-md hover:shadow-lg transition-all space-y-4 flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    {/* Card Header */}
                    <div className="flex items-start justify-between border-b border-amber-900/10 pb-3">
                      <div>
                        <span className="text-[10px] uppercase font-bold tracking-wider text-amber-400 block font-serif">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          title="Recite Mantra"
                          className="p-1.5 rounded-lg bg-amber-500/20 text-amber-200 hover:bg-amber-500/30"
                        >
                        ...
                        >
                          <Bookmark
                            className={`w-3.5 h-3.5 ${
                              isBookmarked(m.id) ? 'fill-current text-amber-400' : ''
                            }`}
                          />
                        </button>
                      </div>
                    ...
                    {/* Sanskrit Box */}
                    <div className="p-4 bg-gradient-to-br from-stone-800 to-stone-900 rounded-xl border border-amber-800/15 text-center space-y-1">
                      <p className="font-serif text-lg sm:text-xl font-bold text-amber-100 sanskrit-font whitespace-pre-line leading-relaxed">
                        {m.sanskrit}
                      </p>
                      <p className="text-[11px] text-amber-300 italic font-serif">
                        {m.transliteration}
                      </p>
                    </div>

                    {/* Translation */}
                    <div className="text-xs font-serif text-stone-200 leading-relaxed">
                      <span className="font-bold text-amber-300">Meaning:</span> "{m.translation}"
                    </div>
